...
async function GetVideos(categoryName, categoryPath){
    return getCategory(categoryName).then(async (category) => {
        if (category.media){
            ...
            fs.writeFile(__dirname + "/VOD/" + (categoryPath + categoryName) + ".json", JSON.stringify(category.media, null, 4), (err) => {
...
GetVideos("VideoOnDemand", "").then((data)=>{
    fs.writeFile(__dirname + "/VOD/All.json", JSON.stringify(foundVideos, null, 4), (err) => {
        if (err) { console.error(err); return; };
        console.log("File has been created");
    });
});